  ...
  try {
    ...
    db.get(
      ...
      (err, session) => {
        ...
        // Stocker les infos de l'utilisateur dans req
        req.user = decoded;
        next();
      }
    );
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    db.get(
      ...
      async (err, userData) => {
        ...
        db.get(
          ...
          [escapeInput(username), escapeInput(email), userId],
          (err, existingUser) => {
            if (err) {
              logToDB(userId, 'PROFILE_UPDATE_ERROR', req.ip, req.headers['user-agent'], { error: err.message });
            ...
            db.run(
              ...
              function(err) {
                ...
                // Mettre à jour les infos dans le token
                const updatedUser = {
                  ...req.user,
                  username,
                  email
                ...
                db.run(
                  ...
                  [token, userId, req.cookies.token],
                  (err) => {
                    if (err) {
                      logToDB(userId, 'SESSION_UPDATE_ERROR', req.ip, req.headers['user-agent'], { error: err.message });
                    }
                    ...
                    logToDB(userId, 'PROFILE_UPDATE_SUCCESS', req.ip, req.headers['user-agent']);
                    
                    // Rediriger vers le profil avec un message de succès
                    req.user = updatedUser; // Mettre à jour l'utilisateur dans la requête
                    res.render('profile', {
    ...
    logToDB(userId, 'PROFILE_UPDATE_ERROR', req.ip, req.headers['user-agent'], { error: error.message });
